...
type
  ...
  public
  class function GetProdutoByID(Conn: TFDConnection; ID: Integer): TFDQuery; static;
...
class function TProdutosDAO.GetProdutoByID(Conn: TFDConnection; ID: Integer): TFDQuery;
begin
  var FDQuery := TFDQuery.Create(nil);
  try
    FDQuery.Connection := Conn;
    FDQuery.SQL.Text := 'SELECT * FROM Produtos WHERE Codigo = :Codigo';
    FDQuery.ParamByName('Codigo').AsInteger := ID;
    FDQuery.Open; // Abra a consulta para obter os dados

    Result := FDQuery;
  except
    FDQuery.Free;
    raise; // Se ocorrer uma exceção, propague-a
  end;
end;
