...
begin
  // Manipulador de evento para edição ao clicar duas vezes
  if not prodGrid.DataSource.DataSet.IsEmpty then
  begin
    // Habilite o modo de edição na célula atual
...
procedure TListaProdutos.prodGridKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  // Manipulador de evento para exclusão ao pressionar "Delete"
  ...
  begin
    ...
    begin
      ...
      begin
        // Chame o método do DAO para excluir o produto pelo Codigo
        TProdutosDAO.RemoverProduto(DataMod.conect, Codigo );
        // Recarregue os dados no DBGrid para refletir a exclusão
        prodGrid.DataSource.DataSet.Refresh;
      end;
    end;
  end;
  // Manipulador de evento para salvar as alterações ao pressionar "Enter"
  if (Key = VK_RETURN) and (prodGrid.EditorMode) then
  begin
    // Salve as alterações
    prodGrid.EditorMode := False;
    prodGrid.Options := prodGrid.Options - [dgEditing];
    prodGrid.DataSource.DataSet.Post; // Isso irá salvar as alterações no banco de dados
  end;
end;
